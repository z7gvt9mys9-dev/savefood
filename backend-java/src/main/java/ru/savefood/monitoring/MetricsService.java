...
/**
 * Port of backend/monitoring.py — Prometheus metrics rendered in the text
 * exposition format with no extra dependency (the Python module used
 * prometheus_client). Records the same series:
 * <ul>
 *   <li>{@code savefood_http_requests_total}{method,route,status} (counter)</li>
 *   <li>{@code savefood_http_request_seconds}{method,route} (histogram)</li>
        ...
        void observe(double seconds) {
            ...
            for (int i = 0; i < BUCKETS.length; i++) {
                if (seconds <= BUCKETS[i]) {
                    buckets[i].increment();
                    break;  // store per-bucket counts; render() makes them cumulative
                }
            }
        }
    }
